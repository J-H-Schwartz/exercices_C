...
const osMessageQueueAttr_t readFromSerial_attributes = {
  .name = "readFromSerial"
};
...
int main(void)
{
  ...
  app_queueHandle = osMessageQueueNew (16, sizeof(Msg_t), &app_queue_attributes);

  /* creation of display_queue */
  display_queueHandle = osMessageQueueNew (16, sizeof(Msg_t), &display_queue_attributes);
