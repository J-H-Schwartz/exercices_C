...
osMessageQueueId_t receiveFromAppHandle;
const osMessageQueueAttr_t receiveFromApp_attributes = { .name =
		"receiveFromApp" };

void SendToSimu(Msg_t* command, size_t size) {
	osMessageQueuePut(receiveFromAppHandle, command, 1, osWaitForever);
}

osSemaphoreId_t start_signalHandle;
const osSemaphoreAttr_t start_signal_attributes = { .name = "start_signal" };
void wait_for_display_init(void) {
	osSemaphoreAcquire(start_signalHandle, osWaitForever);
}

osSemaphoreId_t reset_signalHandle;
const osSemaphoreAttr_t reset_signal_attributes = { .name = "reset_signal" };
void wait_for_display_reset(void) {
	osSemaphoreAcquire(reset_signalHandle, osWaitForever);
}

osSemaphoreId_t valid_signalHandle;
const osSemaphoreAttr_t valid_signal_attributes = { .name = "valid_signal" };
...
void validation_anim_handler(void* callback_param) {
	...
	if (valid_infos->coords_table[coords_table_row_index] != -1) {
		...
		coords_table_row_index = 2;
		osSemaphoreRelease(valid_signalHandle);
	}
}
...
static void gp4_move_down_animation(Msg_t* tmp_message_display) {
	...
	timer_init(&valid_infos.timer, 2, &valid_infos);
	...
	start_timer(&valid_infos.timer);
	osSemaphoreAcquire(valid_signalHandle, osWaitForever);
}
...
static void wait_animation(int color) {
	...
	blink_table[38] = -1;
	blink_animation(blink_table, color, 500, 3);
}
...
void connect_4_message_processing(Msg_t* tmp_message_display,
	...
	if (tmp_message_display->status == RESET_GAME) {
		reset_matrix_animation();
		return;
	}
...
void thread_handler_display(void* args) {
	receiveFromAppHandle = osMessageQueueNew(16, sizeof(Msg_t),
			&receiveFromApp_attributes);
	...
	start_signalHandle = osSemaphoreNew(0, 0, &start_signal_attributes);
	reset_signalHandle = osSemaphoreNew(0, 0, &reset_signal_attributes);
	valid_signalHandle = osSemaphoreNew(0, 0, &valid_signal_attributes);
	while (1) {
		display_restart = 0;
		if (osMessageQueueGet(receiveFromAppHandle, &tmp_message_display, NULL,
				0) == 0) {
			//countdown();
			...
			while (display_restart == 0) {
				...
				blink_infos.color =
						tmp_message_display.move_coordinates.player_color;
				start_timer(&blink_infos.timer);
				if (osMessageQueueGet(receiveFromAppHandle,
						&tmp_message_display, NULL, osWaitForever) == 0) {
